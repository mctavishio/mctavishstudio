...
let createstreams = z => {
	...
	(function() {
		...
		z.streams[name] = Kefir.combine([z.streams["tick"].filter( e => e.t%dt===0 && e.changed )], [z.streams["palette"], z.streams["canvas"]], (tick, palette, canvas) => { return {tick:tick, palette:palette, canvas:canvas } })
			.scan( (state, e) => { 
				state.tick = e.tick;
				state.palette = e.palette;
				state.canvas = e.canvas;
				...
				e.elements.forEach( (row, r) => {
					...
					e.elements[r].forEach( (col,c) => {
						...
						Velocity({	
							...
							properties: { fillOpacity: 1.0, strokeOpacity: 0.0, stroke: e.palette.colors[z.tools.randominteger(0,e.palette.colors.length)], strokeWidth: 12, fill: color, x: x, y: y, width: dx*ratios[z.tools.randominteger(0,ratios.length)]/10, height: dy*ratios[z.tools.randominteger(0,ratios.length)] },
							options: { duration: z.tools.randominteger(e.dt*140,e.dt*600),  delay: z.tools.randominteger(e.dt*100,e.dt*400), easing: "easeInOutQuad" },
						});
					})
